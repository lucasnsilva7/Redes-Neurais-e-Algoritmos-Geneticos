...
def gene_palindromo(letras_possiveis): # Apenas o nome da função mudou
    """Sorteia uma letra.
...
def cria_candidato_palindromo(tamanho_palavra, letras_possiveis):
...
def populacao_palindromo(tamanho_populacao, tamanho_palavra, letras_possiveis):
    """Cria população inicial no problema dos palindromos

    Args
      tamanho_populacao: tamanho da população.
      tamanho_palindromos: inteiro representando o tamanho dos palindromos.
      letras_possiveis: letras possíveis de serem sorteadas.
    ...
    for _ in range(tamanho_populacao):
        populacao.append(cria_candidato_palindromo(tamanho_palavra, letras_possiveis))
...
def funcao_objetivo_palindromo(candidato):
    ...
    Args:
      candidato: um candidato para ser um palindromo que contenha uma vogal

    """
    
    distancia = 0
    vogais = ["a", "e", "i", "o", "u"]
    #candidato_reverso = candidato.copy()
    #candidato_reverso.reverse() 
    ...
    if possui_vogal:
        for letra_candidato, complementar, i in zip(candidato, candidato[::-1], range(len(candidato))): 
            num_letra_candidato = ord(letra_candidato)
            num_complementar = ord(complementar)
            distancia += abs(num_letra_candidato - num_complementar)
            if i == math.ceil(len(candidato)/2): # Math.ceil arredonda para cima
                break
    else:
        distancia = 0.01
